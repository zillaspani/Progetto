...
openssl req -config "$DIR/openssl_ca.cnf" -x509 -newkey rsa:2048 -nodes -keyout tmp_ca.key -out ca-cert_dhe.pem -days 3650

# Generate a certificate request
echo Generating certificate request...; echo
openssl req -config "$DIR/openssl_server.cnf" -newkey rsa:2048 -nodes -keyout tmp_server.key -out tmp_server.req

# Sign the request with the certificate authority's certificate created above
echo Signing certificate request...; echo
openssl x509 -req -in tmp_server.req -CA ca-cert_dhe.pem -CAkey tmp_ca.key -CAcreateserial -days 3650 -out server-cert_dhe.pem

# Build pem file with private and public keys, ready for unprompted server use
cat tmp_server.key server-cert.pem > keycert_dhe.pem

# Clean up
rm tmp_ca.key tmp_server.key tmp_server.req ca-cert_dhe.srl
