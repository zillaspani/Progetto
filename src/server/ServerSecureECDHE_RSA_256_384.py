...
def check_signature(public_key_client, signature_bytes, message):
...
class Handshake(resource.Resource):
    ...
    async def render_get(self,request):
        try:
            ...
            session_key_aes = key_agreement(static_priv=server_private_key_aes, static_pub=client_pubblic_key_aes, kdf=self.kdf32)
            ...
            check_signature(public_key_sender,signature,message)
